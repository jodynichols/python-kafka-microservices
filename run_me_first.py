...
TOPIC_ORDERED = SYS_CONFIG["kafka-topics"]["tea_ordered"]
...
TOPIC_TOPPED = SYS_CONFIG["kafka-topics"]["tea_topped"]
STREAM_TOPPED = TOPIC_TOPPED.replace("-", "_").upper()

TOPIC_STATUS = SYS_CONFIG["kafka-topics"]["tea_status"]
STREAM_STATUS = TOPIC_STATUS.replace("-", "_").upper()

# TOPIC_STATUS_SESSION = SYS_CONFIG["kafka-topics"]["tea_status_session"]
...
KSQL_STATEMENTS = {
    ...
    STREAM_MIXED: f"""CREATE STREAM IF NOT EXISTS {STREAM_MIXED} (
        order_id VARCHAR KEY,
        status INT,
        timestamp BIGINT
    ) WITH (
    ...
    STREAM_TOPPED: f"""CREATE STREAM IF NOT EXISTS {STREAM_TOPPED} (
        order_id VARCHAR KEY,
        status INT,
        timestamp BIGINT
    ) WITH (
